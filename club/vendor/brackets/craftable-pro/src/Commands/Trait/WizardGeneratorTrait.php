...
trait WizardGeneratorTrait
{
    ...
    protected function getAndSetTranslatableColumns()
    {
        ...
        $jsonColumnsNames = $this->tableColumns
            ->whereIn('type', ['json', 'jsonb', 'longtext'])
    ...
    protected function getAndSetRelations()
    {
        ...
        if ($this->shouldAskForOptionValue('add-relation')) {
            ...
            if ($this->components->choice(
                ...
                $intColumns = $this->tableColumns->whereIn('type', ['int', 'integer', 'bigint'])->map(fn ($column) => $column['name']);
                ...
                collect([
                    ...
                    "Brackets\CraftablePro\Models\CraftableProUser" => "CraftableProUser",
                ...
                if(str_contains($relation['namespace'], '\\') && ! str_starts_with($relation['namespace'], "\\")) {
                ...
                if ($customRelatedModel) {
                    ...
                    $relationIntColumns = $relationTableColumns->whereIn('type', ['int', 'integer', 'bigint'])->map(fn ($column) => $column['name'])->values()->all();
                    $relationStringColumns = $relationTableColumns->whereIn('type', ['string', 'json', 'jsonb'])->map(fn ($column) => $column['name'])->values()->all();
                    ...
                    $selectedRelationModel = $models->search($relation['model']);

                    // Set if selected label is translatable to false by default
                    $relation['isLabelTranslatable'] = false;
                    ...
                    if (in_array('Spatie\Translatable\HasTranslations', class_uses_recursive($selectedRelationModel))) {
                        ...
                        $relation['isLabelTranslatable'] = in_array($relation['label'], (new $selectedRelationModel())->getTranslatableAttributes());
                    }
                }
