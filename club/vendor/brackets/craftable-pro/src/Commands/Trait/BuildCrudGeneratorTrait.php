...
trait BuildCrudGeneratorTrait
{
    ...
    protected function requestBuildReplacements(string $requestClassName, string $request, string $permissionName): array
    {
        ...
        return array_merge($this->baseBuildReplacements(), [
            ...
            'requestNamespace' => $this->requestNamespace,
            'rules' => $rules,
            'permissionName' => "craftable-pro." . Str::kebab($this->className) . ".$permissionName",
        ]);
    }
    ...
    protected function viewsBuildReplacements(): array
    {
        ...
        return [
            ...
            '[[formTranslatableFunctionality]]' => $this->buildFormTranslatableFunctionality(),
        ];
    }
    ...
    protected function buildPermissionMigrationReplace(): array
    {
        return [
            ...
            collect(['index', 'create', 'edit', 'destroy'])->map(function ($permission) {
                return "craftable-pro." . Str::kebab($this->className) . ".$permission";
            }),
    ...
    protected function buildVueIndexColumns(): mixed
    {
        return $this->indexColumns->map(function ($column) {
            ...
            if ($showDateFormat) {
                $dataCellContent = $column['type'] === 'time'
                    ? " {{ dayjs(item.{$column["name"]}, 'HH:mm').format('$showDateFormat') }}"
                    : " {{ dayjs(item.{$column["name"]}).format('$showDateFormat') }}";
            }

            if ($relation = $this->relations?->firstWhere(fn ($relation) => $relation['foreignKey'] === $column['name'] && $relation['type'] === 'belongsTo')) {
                if ($relation['isLabelTranslatable']) {
                    $dataCellContent = ' {{ item.'.Str::snake($relation['model']).'.'.$relation['label'].'?.[currentLocale] }}';
                } else {
            ...
            if ($column['type'] === 'boolean') {
                $dataCellContent =
                    '<ListingToggle name="'.$column['name'].'" v-model="item.'.$column['name'].'" :updateUrl="route(\'craftable-pro.' . $updateRoute . '\', item.id)" />';
            }
...
const { availableLocales, defaultLocale, currentLocale, translatableDefaultValue, getLabelWithLocale } = useFormLocale(); \n\n
    ...
    {
        if (count($this->translatable) > 0) {
            return '<CardLocaleSwitcher v-model="selectedLocale" class="mb-6" />';
        }
    ...
    protected function mapRelationsData(array $columns): Collection
    {
        return collect($columns)->map(function ($column) {
            ...
            $selectedRelationModel = "App\\Models\\$relatedModel";

            // Set if selected label is translatable to false by default
            $isLabelTranslatable = false;

            // Check if selected relation model has "Spatie\Translatable\HasTranslations" -> is translatable
            if (in_array('Spatie\Translatable\HasTranslations', class_uses_recursive($selectedRelationModel))) {

                // If selected relation model is translatable, check if selected label is translatable
                $isLabelTranslatable = in_array($modelAccesor, (new $selectedRelationModel())->getTranslatableAttributes());
            }
    ...
    protected function buildFormTranslatableFunctionality(): string
    {
        if (count($this->translatable) > 0) {
            return "
