...
trait CrudGeneratorTrait
{
    ...
    {
        $tables = collect($this->databaseManager->listTableNames())->filter(function ($table) {
    ...
    protected function getTableColumns(string $tableName)
    {
        return collect(Schema::getColumnListing($tableName))->map(function ($column) use ($tableName) {
            ...
            $columnType = $this->databaseManager->getColumnType($tableName, $column);
            $columnRequired = $this->databaseManager->getNotNull($tableName, $column);
