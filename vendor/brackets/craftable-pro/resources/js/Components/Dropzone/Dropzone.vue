      ...
      >
        ...
        >
          <slot
            ...
            :max-file-size="maxFileSizeWithDefault"
          ...
          >
            ...
            <div class="space-y-1">
              ...
              <p class="text-xs text-gray-500">
                ...
                <span v-if="maxFileSizeWithDefault">
                  up to {{ formatBytes(maxFileSizeWithDefault) }}
                </span>
              </p>
            </div>
          </slot>
...
const maxFileSizeWithDefault = computed(() => {
  if (props.maxFileSize) {
    return props.maxFileSize;
  }

  const page = usePage();
  return page.props.config?.media_library?.max_file_size ?? 2 * 1024 * 1024;
});
...
const uploadFiles = (filesToUpload: File[]) => {
  ...
  filesToUpload.forEach((file: File) => {
    if (file.size > maxFileSizeWithDefault.value!) {
...
const switchFile = (uuid: string, e: HTMLInputElement) => {
  ...
  if (fileToSwitch) {
    if (fileToSwitch.size > maxFileSizeWithDefault.value!) {
      toast.error(trans("craftable-pro", "File size is too big!"));
      return;
    }
